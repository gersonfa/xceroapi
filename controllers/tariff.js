...
async function tariff_list(req, res, next) {
  try {
    ...
    if (base_id && (group_id || place_id)) {
      if (group_id) {
        query.$or = [{ origin_group: group_id }, { destiny_group: group_id }]
      } else if (place_id) {
        query.$or = [{ origin_place: place_id }, { destiny_place: place_id }]
      }

      tariffs = await Tariff.find(query).populate([
        { path: 'origin_group', populate: { path: 'base', select: 'name' } },
        { path: 'destiny_group', populate: { path: 'base', select: 'name' } },
      ...
      tariffs = await Tariff.find(query)
...
async function tariff_update(req, res, next) {
  try {
    ...
    let tariff = await Tariff.findByIdAndUpdate(tariff_id, req.body, {
      new: true
    })
...
async function tariff_details(req, res, next) {
  try {
    ...
    let tariff = await Tariff.findById(tariff_id).populate([
      { path: 'origin_group', populate: { path: 'base', select: 'name' } },
      { path: 'destiny_group', populate: { path: 'base', select: 'name' } }
    ])

...
async function group_place_available(req, res, next) {
  try {
    ...
    if (group_id) {
      let tariff_same_place = tariffs.find(
        t =>
          t.origin_group &&
          t.origin_group.toString() === group_id &&
          (t.destiny_group && t.destiny_group.toString() === group_id)
      );
      if (tariff_same_place) {
        groups_places = groups_places.filter(
          gp => gp._id.toString() != group_id
        )
      }
      ...
      groups_places = groups_places.filter(gp => {
        let tariff = tariffs.find(t => {
          if (
            ...
            t.origin_group.toString() === group_id &&
            ((t.destiny_group &&
              t.destiny_group.toString() === gp._id.toString()) ||
              (t.destiny_place &&
                t.destiny_place.toString() === gp._id.toString()))
          ) {
            return t
          } else if (
            t.destiny_group &&
            t.destiny_group.toString() === group_id &&
            ((t.origin_group &&
              t.origin_group.toString() === gp._id.toString()) ||
              (t.origin_place &&
                t.origin_place.toString() === gp._id.toString()))
          ) {
            return t
          }
        });
...
async function group_delete(req, res, next) {
  try {
    ...
    await Tariff.deleteMany({
      $or: [{ origin_group: group_id }, { destiny_group: group_id }]
    });
